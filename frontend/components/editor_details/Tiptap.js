...
export default function Tiptap({ params, data_content }) {
    ...
    */
    ...
    const handleCopyBtn = async () => {
        const req_insert_body = {
            "id": params,
            "updated": JSON.parse(fmt_out(editor.getHTML()).substring(2,
            fmt_out(editor.getHTML()).length))
        }
        console.log(JSON.stringify(req_insert_body, null, 4))
    ...
    const extract_content = (data_content) => {
        let ret_str = ''
        Object.keys(data_content).map(((line) => {
            Object.keys(data_content[line]).map((sp) => {
                const lead_part = data_content[line][sp].substring(0, 3)
                if (lead_part == '!ty') {
                    ret_str += data_content[line][sp].substring(3,
                        data_content[line][sp].length)
                            .replace(/ /g, '\u2022')
                } else if (lead_part == '!sk') {
                    ret_str += (''
                        + '<span style="color: rgb(204, 204, 204);">'
                        + data_content[line][sp].substring(3, data_content[line][sp].length)
                            .replace(/ /g, '\u2022')
                        + '</span>')
                }
            })
    ...
    const editor = useEditor({
        ...
        content: (`<p>${extract_content(data_content["content"])}</p>`),
        editorProps: {
            attributes: {
                class:
            ...
            <div>
                <p>
                    ...
                    <input
                        ...
                        placeholder="label"
                        defaultValue={data_content.title}
                    />
                    <button
